        ...
        while read_position < len(mml):
            ...
            if note_match:
                ...
                if note_code == "r":
                    ...
                    ##print  "note_code=",note_code

                    # 半音符号の処理
                    if accidential:
                        if accidential in "+": ##+ってなってたんだけど、多分いらないのでは. おそらく#って書いても表現できるよってことだと思われ
                            note_diff += 1
                        elif accidential == "-":
                            note_diff -= 1

                    ##print  "self.tuning = " + str(self.tuning) + "\nself.octave = " + str(self.octave) + "\nself.note_diff = " + str(note_diff)
                    ##print  "1. ", 
                    # 周波数の取得(平均12律)
                    note_frequency = self.tuning * \
                                     (2 ** (self.octave - 4)) * \
                                     ((2 ** note_diff) ** (1 / 12.0))
                    #print  "note_frequency = ", note_frequency
            ...
            if mml[read_position] in "<>":
                ...
                if mml[read_position] == ">":
                    ##print  "> detected"
                    ##print  "before: ", self.octave
                    self.octave += 0.5
                    ##print  "after: ", self.octave
                    ##print  "[+] Octave Updated: " + str(self.octave)
                else:
                    ##print  "< detected"
                    self.octave -= 0.5
                    ##print  "[-] Octave Updated: " + str(self.octave)
