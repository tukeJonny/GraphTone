...
class SquareWaveOscillator(Oscillator):
    ...
    def get_value(self, tick):
        getcontext().prec = 100
        l = Decimal(Config.SampleRate) / Decimal(self.frequency)
        #print "Config.SampleRate = " + str(Config.SampleRate)
        #print "self.freq =  " + str(self.frequency)
        #print "l = " + str(l)
        change_point = Decimal(l) * Decimal(self.duty)
        try:
            ...
            print "Raise ZeroDivision Error"
...
class Renderer(object):
    def __init__(self, clock=None, source=None, sink=None):
        ...
        self.source = source
        self.sink = sink
        print "Initted source(sequence) = " + str(self.source.sequences)

        #print "*"*30 + " DEBUG Renderer " + "*"*30
        #print "clock = " + str(self.clock)
        #self.source.debug_args()
